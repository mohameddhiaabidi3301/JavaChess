...
public class SearchThread extends Thread {
	...
	public SearchThread(int initializerMove, int depth, boolean isMaximizing, int alpha, int beta, int[] previousBestMove, long itdStartTime, int maxTime, int quiessenceCount, int ply, CountDownLatch latch) {
	...
	private boolean hasTimedOut(long start, int maxTimeMs) {
	    return (System.nanoTime() - start) >= (maxTimeMs * 1_000_000L);
	}
	...
	public int[] minimax(int initializerMove, int depth, boolean isMaximizing, int alpha, int beta, int[] previousBestMove, long itdStartTime, int maxTime, int quiessenceCount, int ply) {
		...
		synchronized (Minimax.zobristValues[TT_INDEX]) {
			if (Minimax.zobristKeys.get(TT_INDEX) == localPosition.zobristHash) {
				if (hasTimedOut(itdStartTime, maxTime)) {
		...
		for (int i = 0; i < moves.length; i++) {
			...
			boolean extendQuiessence = ((isCapture || isPromotion || usInCheck) && quiessenceCount < 8);
			boolean lateMoveReduction = !extendQuiessence && depth >= 3 && i > 2;
			int reducedDepth = (lateMoveReduction) ? depth - 1 : depth; // LMR
			
			int nextDepth = (extendQuiessence) ? reducedDepth : reducedDepth - 1;
			...
			if (!isPromotion) {
				...
				if (lateMoveReduction && score[1] > alpha) {
					score = minimax(-1, (extendQuiessence ? depth : depth - 1), !isMaximizing, alpha, beta, null, itdStartTime, maxTime, nextQuiessence, ply + 1);
				}
		...
		if (bestMove[2] != Minimax.FLAG_TIMEOUT) {
			if (Minimax.zobristKeys.get(TT_INDEX) == localPosition.zobristHash) {
				...
				Minimax.zobristKeys.set(TT_INDEX, localPosition.zobristHash);
				Minimax.zobristValues[TT_INDEX] = bestMove;
			}
		}
